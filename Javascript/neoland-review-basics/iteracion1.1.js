...
for (const keys of movies) {
  if (keys.categories.includes (`comedia`)){   
     moviesComedia.push(keys);
  }else if(keys.categories.includes (`acción`)){
     moviesAccion.push(keys);}
    else if(keys.categories.includes (`aventura`)){
        moviesAventura.push(keys)}
    
  
}
console.log(moviesComedia);
console.log(moviesAccion);
console.log(moviesAventura);
